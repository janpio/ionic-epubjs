...
EPUBJS.core.request = function(url, type, withCredentials) {
	...
	var handler = function() {
		...
		if ((this.status === 200 || this.status === 0) && this.response) { // Android & Firefox reporting 0 for localFormat & blob urls
			if (type == 'xml'){
...
EPUBJS.Render.Iframe.prototype.resized = function(e){
  // TODO Modified by Valentin
	// Get the fractional height and width of the iframe
	// this.width = this.iframe.getBoundingClientRect().width;
	// this.height = this.iframe.getBoundingClientRect().height;
  this.width = parseInt(window.getComputedStyle(document.getElementById('area')).width);
  this.height = parseInt(window.getComputedStyle(document.getElementById('area')).height);
...
EPUBJS.Renderer.prototype.onResized = function(e) {
  ...
  var width = parseInt(window.getComputedStyle(document.getElementById('area')).width),
	...
	var replacments = function(link, done){
		...
		if(isRelative != -1){
			...
			if(directory) {
				...
				// localFormat file links, as in certain contexts (such as under
